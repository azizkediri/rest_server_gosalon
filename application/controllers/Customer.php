...
class Customer extends REST_Controller {
	...
	function index_get(){
		$get_customer = $this->db->query("SELECT nama_cust, alamat, jenis_kelamin, photo, password FROM customer")->result();
		$this->response(array("status"=>"success","result" => $get_customer));
	}
	function index_post() {
	...
	function insertCustomer($data_customer){
		...
		$uploaddir = str_replace("application/", "", APPPATH).'upload/customer';
		...
		if (!empty($_FILES)){
			...
			$data_customer['photo'] = "upload/customer".$user_img;
		}else{
			$data_customer['photo']="";
		}
		//////////////////////////////////////////////////////////////////
		...
		if (empty($data_customer['nama_cust'])){
			...
			if (!empty($_FILES)){
				if ($_FILES["photo"]["name"]) {
					...
					$data_customer['photo'] = base_url()."upload/customer".$user_img;
				}else{
						$data_customer['photo'] = "";
					}
					if ($insert){
	...
	//function upload image
		...
		//$this->response(array(base_url()."upload/customer".$user_img));
		//////////////////////////////////////////////////////////////////
		//////////////////////////////////////////////////////////////////
		//cek validasi
